...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const breakpoint = 900; // ブレークポイント（画面幅）
...
// swiper(spot)
const swiper_spot = new Swiper(".swiper_spot", {
  loop: true,
  loopedSlides: 10,
...
$(function () {
  // １つ目のアコーディオンのみ開いておく
  $(".accordion_list:first-child .accordion_box").css(
    "display", "block");
  $(".accordion_list:first-child .accordion_toggle").addClass(
    "close");

  $(".accordion_area").on("click", function () {
    const accordion = $(this).next(".accordion_box");
    const toggle = $(this).find(".accordion_toggle");
...
// フォームバリデーション //
$(function () {
  const form = jQuery("#js_form");
  const inputElements = form.find(".js_input");
